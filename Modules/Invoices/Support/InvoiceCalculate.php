...
class InvoiceCalculate
{
    ...
    public function calculate($invoice)
    {
        ...
        if ($calculatedAmount['total'] > 0 && $calculatedAmount['balance'] <= 0 && $invoice->status_text != 'canceled') {
        ...
        if ($calculatedAmount['total'] > 0 && $calculatedAmount['balance'] > 0 && $invoice->invoice_status_id == InvoiceStatuses::getStatusId('paid')) {
            $invoice->invoice_status_id = InvoiceStatuses::getStatusId('is_sent');
            $invoice->save();
        }
    }
}
