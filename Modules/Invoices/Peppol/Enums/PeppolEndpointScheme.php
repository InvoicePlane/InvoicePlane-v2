...
enum PeppolEndpointScheme: string
{
    ...
    public function validates(string $identifier): bool
    {
        ...
        return match ($this) {
            ...
            self::IT_CF => (bool) preg_match('/^[A-Z0-9]{16}$/', strtoupper($identifier)),
            self::ES_VAT => (bool) preg_match('/^[A-Z]\d{7,8}[A-Z0-9]$/', strtoupper($identifier)),
            ...
            self::GB_COH => (bool) preg_match('/^[A-Z0-9]{8}$/', strtoupper($identifier)),
            self::GLN => (bool) preg_match('/^\d{13}$/', $identifier),
            self::DUNS => (bool) preg_match('/^\d{9}$/', $identifier),
            self::ISO_6523 => strlen($identifier) > 0, // Flexible validation
        };
    }
